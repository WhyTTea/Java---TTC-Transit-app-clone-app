...
public class Cardholder{
	...
	public String Exit(Stop curr_stop, Date today, Transit_System ts) {
		...
		if ((curr_stop.getType() == "Bus") && (curr_trip.checkValidExit() == true)) {
			...
			if (numberSubwaySt == -1) {curr_card.completeMaxBusTransaction();
			return ("You forgot to tap out at one of the stop in the middle of your route. You were charged daily maximum fee. Balance:" + curr_card.getBalance());}
		}
		else if((curr_stop.getType() == "Bus") && (curr_trip.checkValidExit() == false)) {
			curr_card.completeBusTransaction();
			return ("It seems you forgot to tap in, we have charged your card for a cost of one bus fare");
		}
			...
			//if (numberSubwaySt = 0) do not charge}
